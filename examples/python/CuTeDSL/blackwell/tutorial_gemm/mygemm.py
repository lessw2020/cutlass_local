...
threads_per_cta = 128  # 4 warps per CTA
#pipeline stages
ab_stages = 4
acc_stage = 1
# tmem
...
# 2 - ab mma pipeline
    ...
    if warp_idx == 0:
        ...
        for k_tile_idx in cutlass.range(num_k_tiles, prefetch_stages=ab_stages - 2):
            ...
            ab_full.release()  # read for next producer load
        acc_empty.commit()
...
def run_dense_gemm(
    ...
    def make_tensors(mn, k, dtype=torch.bfloat16):
        shape = (mn, k)
        return (
...
if __name__ == "__main__":
    ...
    run_dense_gemm(mnk, tolerance)
    print("PASS with first CuteDSL implementation for B200+ GEMM")