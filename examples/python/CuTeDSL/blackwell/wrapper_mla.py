...
class CuteDSLMLAWrapper:
    ...
    def __call__(
        ...
        # q_nope: (bsz, num_heads, 1, qk_nope_head_dim)
        # Squeeze and permute to (num_heads, bsz, qk_nope_head_dim)
        q_nope_flat = q_nope.squeeze(2).permute(1, 0, 2)
        
        # latent_to_k: (latent_dim, num_heads, qk_nope_head_dim)
        # Permute to (num_heads, qk_nope_head_dim, latent_dim)
        latent_to_k_bmm = latent_to_k.permute(1, 2, 0)
        
        # q_latent: (num_heads, bsz, latent_dim)
        q_latent = torch.bmm(q_nope_flat, latent_to_k_bmm)
        
        # CuteDSL kernel expects stride[1] == 1, so we need (num_heads, latent_dim, bsz)
        # where latent_dim is contiguous. We'll create a view with non-standard strides.
        q_latent = q_latent.contiguous()  # (num_heads, bsz, latent_dim) row-major
        
        # Prepare q_pe: (bsz, num_heads, 1, rope_dim) -> (num_heads, bsz, rope_dim)
        q_pe_cute = q_pe.squeeze(2).permute(1, 0, 2).contiguous()
        
        # Prepare KV cache: (bsz, k_len, dim) -> (k_len, bsz, dim)  
        c_latent_cute = kv_latents.transpose(0, 1).contiguous()
        k_pe_cute = k_pe.transpose(0, 1).contiguous()
        ...
        # Note: will be transposed later to get proper stride layout
        # Use the same dtype as input
        torch_dtype = q_nope.dtype
        o_torch = torch.empty((num_heads, bsz, self.latent_dim), dtype=torch_dtype, device=q_nope.device)
        lse_torch = torch.empty((bsz, num_heads), dtype=torch.float32, device=q_nope.device)
        ...
        # Reinterpret tensors to have stride[1] == 1 (middle dimension contiguous)
        # From (num_heads, bsz, latent_dim) to (num_heads, latent_dim, bsz) with stride[1] == 1
        q_latent_view = q_latent.transpose(1, 2)  # Now has stride[1] == 1
        q_pe_view = q_pe_cute.transpose(1, 2)
        c_latent_view = c_latent_cute.transpose(1, 2)
        k_pe_view = k_pe_cute.transpose(1, 2)
        o_view = o_torch.transpose(1, 2)
        
        # Convert to CuTe tensors
        q_latent_cute = self._convert_to_cute(q_latent_view, self.in_dtype)
        q_latent_cute = q_latent_cute.mark_layout_dynamic(leading_dim=1)
        
        q_pe_cute_t = self._convert_to_cute(q_pe_view, self.in_dtype)
        q_pe_cute_t = q_pe_cute_t.mark_layout_dynamic(leading_dim=1)
        
        c_latent_cute_t = self._convert_to_cute(c_latent_view, self.in_dtype)
        c_latent_cute_t = c_latent_cute_t.mark_layout_dynamic(leading_dim=1)
        
        k_pe_cute_t = self._convert_to_cute(k_pe_view, self.in_dtype)
        k_pe_cute_t = k_pe_cute_t.mark_layout_dynamic(leading_dim=1)
        
        o_cute = self._convert_to_cute(o_view, self.out_dtype)
        o_cute = o_cute.mark_layout_dynamic(leading_dim=1)
        
        # LSE needs to be transposed to (num_heads, bsz) with stride[0] == 1
        lse_view = lse_torch.T  # (num_heads, bsz) with stride[0] == 1
        lse_cute = self._convert_to_cute(lse_view, self.lse_dtype)
        ...
        # o_torch: (num_heads, bsz, latent_dim) - already in the right layout!
        
        # latent_to_v: (latent_dim, num_heads, value_dim)
        # Permute to (num_heads, latent_dim, value_dim)
        latent_to_v_bmm = latent_to_v.permute(1, 0, 2)
        
        # o_final: (num_heads, bsz, value_dim)
        o_final = torch.bmm(o_torch, latent_to_v_bmm)
        
        # Reshape to (bsz, num_heads * value_dim)
...
def test_decode_step():
    ...
    device = "cuda"
    dtype = torch.float16  # Must match cutlass.Float16
